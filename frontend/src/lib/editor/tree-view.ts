import type { Block, BlockType } from "@notion-clone/contracts";
...
export function visibleBlocks(tree: BlockTree, collapsed: ReadonlySet<string>): VisibleBlock[] {
  const rows: VisibleBlock[] = [];

  function walk(parentId: string, depth: number) {
    getChildren(tree, parentId).forEach((block, index) => {
      rows.push({ block, depth, parentId, index });
      if (block.type !== "toggle" || !collapsed.has(block.id)) walk(block.id, depth + 1);
    });
  }
...
export function isDescendantOf(tree: BlockTree, possibleDescendantId: string, ancestorId: string): boolean {
