...
// Engine puro do M1: aplica as cinco operações numa árvore em memória e devolve
// a árvore nova + as ops inversas (base do undo). O servidor (M2) reimplementa
// esta mesma semântica em Rust; os testes daqui viram o contrato de comportamento.
...
export const WORKSPACE_ID = "local"; // ponytail: M1 é single-workspace em memória; M2 troca por id real.
...
export function createPageTree(title: string, rootId: string = crypto.randomUUID()): BlockTree {
...
function isDescendant(tree: BlockTree, ancestorId: string, id: string): boolean {
...
export function applyOperation(
  ...
  switch (op.type) {
    case "insert_block": {
      if (tree.blocks.has(op.block.id)) throw new EngineError(`duplicate block id: ${op.block.id}`);
      // ponytail: insert é sempre de bloco folha; subárvores entram como uma op por bloco.
      if (op.block.content.length > 0) throw new EngineError("insert_block requires empty content");
      const parent = getBlock(tree, op.parentId);
      if (parent.trashedAt) throw new EngineError("cannot insert into trashed block");
      const index = clampIndex(op.index, parent.content.length);
      const content = [...parent.content];
      content.splice(index, 0, op.block.id);
      const inserted: Block = { ...op.block, parentId: op.parentId, trashedAt: null, trashedIndex: null };
      return {
        ...
        inverse: [{ type: "delete_block", opId: crypto.randomUUID(), blockId: op.block.id }],
      };
    }
    ...
    case "update_block": {
      ...
      const inverse: UpdateBlockOp = { type: "update_block", opId: crypto.randomUUID(), blockId: op.blockId };
      ...
      if (op.properties) {
        ...
        for (const [key, value] of Object.entries(op.properties)) {
          inverse.properties[key] = key in block.properties ? (block.properties[key] as JsonValue) : null;
    ...
    case "move_block": {
      ...
      if (op.blockId === op.newParentId) throw new EngineError("cannot move block into itself");
      if (isDescendant(tree, op.blockId, op.newParentId)) throw new EngineError("move would create a cycle");
      const newParent = getBlock(tree, op.newParentId);
      if (newParent.trashedAt) throw new EngineError("cannot move into trashed block");

      const oldParent = getBlock(tree, block.parentId);
      const oldIndex = oldParent.content.indexOf(op.blockId);
      if (oldIndex === -1) throw new EngineError(`content/parentId mismatch for ${op.blockId}`);
      ...
      const changed = sameParent
        ...
        : [moved, { ...oldParent, content: oldContent }, { ...newParent, content: newContent }];
      return {
        tree: mutate(tree, ...changed),
        inverse: [
          { type: "move_block", opId: crypto.randomUUID(), blockId: op.blockId, newParentId: oldParent.id, index: oldIndex },
        ],
      };
    }
    ...
    case "delete_block": {
      ...
      if (index === -1) throw new EngineError(`content/parentId mismatch for ${op.blockId}`);
      const content = [...parent.content];
      content.splice(index, 1);
      return {
        tree: mutate(tree, { ...block, trashedAt: now(), trashedIndex: index }, { ...parent, content }),
        inverse: [{ type: "restore_block", opId: crypto.randomUUID(), blockId: op.blockId }],
      };
    }

    case "restore_block": {
      const block = getBlock(tree, op.blockId);
      if (!block.trashedAt || block.parentId === null) throw new EngineError("block is not trashed");
      // O pai pode estar trashed (deletado depois do filho): o restore ainda vale,
      // o bloco só fica visível quando o ancestral voltar.
      const parent = getBlock(tree, block.parentId);
      const index = clampIndex(block.trashedIndex ?? parent.content.length, parent.content.length);
      const content = [...parent.content];
      content.splice(index, 0, op.blockId);
      return {
        tree: mutate(tree, { ...block, trashedAt: null, trashedIndex: null }, { ...parent, content }),
        inverse: [{ type: "delete_block", opId: crypto.randomUUID(), blockId: op.blockId }],
      };
    }
  }
}
...
export function visibleTree(tree: BlockTree, id: string = tree.rootId): VisibleNode {
...
export function checkInvariants(tree: BlockTree): void {
  ...
  if (root.parentId !== null) throw new EngineError("root must have null parentId");
  for (const block of tree.blocks.values()) {
    for (const childId of block.content) {
      const child = getBlock(tree, childId);
      if (child.parentId !== block.id)
        throw new EngineError(`content/parentId mismatch: ${childId} in ${block.id}`);
      if (child.trashedAt !== null)
        throw new EngineError(`trashed block ${childId} still in content of ${block.id}`);
    }
    if (block.id !== tree.rootId) {
      ...
      if (block.trashedAt !== null && inParent)
        throw new EngineError(`trashed block ${block.id} still in parent content`);
      if (isDescendant(tree, block.id, block.id)) throw new EngineError(`cycle at ${block.id}`);
    }
  }
}
