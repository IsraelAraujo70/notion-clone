...
import type { Block, Operation } from "@notion-clone/contracts";
import {
  applyAll,
  applyOperation,
  checkInvariants,
...
function fixture() {
  ...
  const [a, b, c] = [newBlock("paragraph", { text: "a" }, "a"), newBlock("paragraph", { text: "b" }, "b"), newBlock("paragraph", { text: "c" }, "c")];
  tree = applyAll(tree, [insertOp(a, "root", 0), insertOp(b, "root", 1), insertOp(c, "root", 2)]).tree;
  return tree;
}
...
describe("insert_block", () => {
  it("inserts at the given index and clamps out-of-range indexes", () => {
    let tree = fixture();
    tree = applyOperation(tree, insertOp(newBlock("paragraph", { text: "x" }, "x"), "root", 1)).tree;
    expect(texts(tree)).toEqual(["a", "x", "b", "c"]);
    tree = applyOperation(tree, insertOp(newBlock("paragraph", { text: "y" }, "y"), "root", 99)).tree;
  ...
  it("rejects duplicate ids, missing parents and non-empty content", () => {
    const tree = fixture();
    expect(() => applyOperation(tree, insertOp(newBlock("paragraph", {}, "a"), "root", 0))).toThrow(EngineError);
    expect(() => applyOperation(tree, insertOp(newBlock("paragraph", {}, "z"), "nope", 0))).toThrow(EngineError);
    const withChildren = { ...newBlock("paragraph", {}, "z"), content: ["a"] };
    expect(() => applyOperation(tree, insertOp(withChildren, "root", 0))).toThrow(EngineError);
  });

  it("inverse (delete) hides the inserted block again", () => {
    const before = fixture();
    const { tree, inverse } = applyOperation(before, insertOp(newBlock("paragraph", { text: "x" }, "x"), "root", 1));
...
describe("update_block", () => {
  it("patches properties, deletes with null, and turns block types in place", () => {
    ...
    tree = applyOperation(tree, {
      type: "update_block", opId: oid(), blockId: "a", blockType: "to_do",
    ...
    tree = applyOperation(tree, { type: "update_block", opId: oid(), blockId: "a", properties: { checked: null } }).tree;
  ...
  it("inverse restores previous type and properties exactly", () => {
    ...
    const { tree, inverse } = applyOperation(before, {
      type: "update_block", opId: oid(), blockId: "b", blockType: "heading1",
...
describe("move_block", () => {
  it("reorders within the same parent and inverse restores order", () => {
    const before = fixture();
    const { tree, inverse } = applyOperation(before, { type: "move_block", opId: oid(), blockId: "a", newParentId: "root", index: 2 });
    expect(texts(tree)).toEqual(["b", "c", "a"]);
    expect(visibleTree(applyAll(tree, inverse).tree)).toEqual(visibleTree(before));
  });

  it("reparents (indent) and inverse restores the original position", () => {
    const before = fixture();
    const { tree, inverse } = applyOperation(before, { type: "move_block", opId: oid(), blockId: "b", newParentId: "a", index: 0 });
  ...
  it("rejects cycles, self-parenting and moving the root", () => {
    let tree = fixture();
    tree = applyOperation(tree, { type: "move_block", opId: oid(), blockId: "b", newParentId: "a", index: 0 }).tree;
    expect(() => applyOperation(tree, { type: "move_block", opId: oid(), blockId: "a", newParentId: "b", index: 0 })).toThrow("cycle");
    expect(() => applyOperation(tree, { type: "move_block", opId: oid(), blockId: "a", newParentId: "a", index: 0 })).toThrow(EngineError);
    expect(() => applyOperation(tree, { type: "move_block", opId: oid(), blockId: "root", newParentId: "a", index: 0 })).toThrow("root");
  });
});
...
describe("delete_block / restore_block", () => {
  it("trashes a subtree, keeps it intact, and restores it in place", () => {
    let tree = fixture();
    tree = applyOperation(tree, { type: "move_block", opId: oid(), blockId: "c", newParentId: "b", index: 0 }).tree;
    const beforeDelete = visibleTree(tree);

    tree = applyOperation(tree, { type: "delete_block", opId: oid(), blockId: "b" }).tree;
    expect(texts(tree)).toEqual(["a"]);
    checkInvariants(tree);

    tree = applyOperation(tree, { type: "restore_block", opId: oid(), blockId: "b" }).tree;
  ...
  it("rejects deleting the root, double delete and restoring live blocks", () => {
    let tree = fixture();
    expect(() => applyOperation(tree, { type: "delete_block", opId: oid(), blockId: "root" })).toThrow("root");
    expect(() => applyOperation(tree, { type: "restore_block", opId: oid(), blockId: "a" })).toThrow(EngineError);
    tree = applyOperation(tree, { type: "delete_block", opId: oid(), blockId: "a" }).tree;
    expect(() => applyOperation(tree, { type: "delete_block", opId: oid(), blockId: "a" })).toThrow(EngineError);
    expect(() => applyOperation(tree, { type: "update_block", opId: oid(), blockId: "a", properties: { text: "x" } })).toThrow(EngineError);
  });
});
...
describe("fuzz: random ops keep invariants and undo returns to the start", () => {
  ...
  it("200 random ops, invariants hold, full undo is exact", () => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      if (roll < 0.35) {
        op = insertOp(newBlock("paragraph", { text: `n${created++}` }, `n${created}`), pick(active), Math.floor(rand() * 5));
      } else if (roll < 0.6 && nonRoot.length) {
        op = { type: "update_block", opId: oid(), blockId: pick(nonRoot), properties: { text: `e${i}` } };
        ...
        if (parents.length) op = { type: "move_block", opId: oid(), blockId, newParentId: pick(parents), index: Math.floor(rand() * 5) };
      } else if (roll < 0.9 && nonRoot.length) {
        const blockId = pick(nonRoot);
        trashed.push(blockId);
        op = { type: "delete_block", opId: oid(), blockId };
      } else if (trashed.length) {
        op = { type: "restore_block", opId: oid(), blockId: trashed.splice(Math.floor(rand() * trashed.length), 1)[0] };
      }
      if (!op) continue;
