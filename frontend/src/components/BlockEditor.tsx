...
import type { Block, BlockType, Operation } from "@notion-clone/contracts";
import {
  KeyboardEvent,
  useCallback,
  useLayoutEffect,
...
import { blockText, isDescendantOf, isTextBlock, siblingIndex, visibleBlocks } from "@/lib/editor/tree-view";
import { detectMarkdownShortcut, removeSlashQuery, slashQuery } from "@/lib/editor/markdown";
...
interface BlockEditorProps {
  ...
  dispatchBatch: (ops: Operation[], options?: { coalesceKey?: string; breakCoalescing?: boolean }) => void;
  undo: () => void;
  redo: () => void;
}

const LIST_TYPES = new Set<BlockType>(["bulleted_list_item", "numbered_list_item", "to_do"]);
...
export function BlockEditor({
  ...
  const rowById = useMemo(() => new Map(rows.map((row, index) => [row.block.id, { ...row, visibleIndex: index }])), [rows]);
  ...
  const insertSibling = useCallback(
    (block: Block, type: BlockType, text = "") => {
      ...
      const parent = getBlock(tree, block.parentId);
      const index = parent.content.indexOf(block.id);
      const fresh = newBlock(type, type === "to_do" ? { text, checked: false } : { text });
      return {
        op: { type: "insert_block", opId: opId(), block: fresh, parentId: parent.id, index: index + 1 } satisfies Operation,
        blockId: fresh.id,
      };
    },
    [tree],
    ...
    (block: Block, element: HTMLElement) => {
      ...
      if (shortcut) {
        if (shortcut.replacesBlock) {
          dispatchBatch(
            [{ type: "update_block", opId: opId(), blockId: block.id, blockType: "divider", properties: { text: null, checked: null } }],
        ...
        requestFocus({ blockId: block.id, offset: shortcut.text.length, forceTextSync: true });
        dispatchBatch(
          [
            {
              type: "update_block",
              ...
              properties: { text: shortcut.text, checked: shortcut.blockType === "to_do" ? false : null },
            },
          ],
          { breakCoalescing: true },
        );
      ...
      dispatchBatch([{ type: "update_block", opId: opId(), blockId: block.id, properties: { text } }], {
        coalesceKey: `text:${block.id}`,
      });
      const query = slashQuery(text, caret);
      setSlash(query === null ? null : { blockId: block.id, query, activeIndex: 0 });
    },
  ...
  const splitBlock = useCallback(
    (block: Block, caret: number) => {
      ...
      if (text.length === 0 && EMPTY_EXIT_TYPES.has(block.type)) {
        ...
        dispatchBatch(
          [{ type: "update_block", opId: opId(), blockId: block.id, blockType: "paragraph", properties: { checked: null } }],
          { breakCoalescing: true },
        );
        return;
      }
      ...
      dispatchBatch(
        [
          { type: "update_block", opId: opId(), blockId: block.id, properties: { text: text.slice(0, caret) } },
          next.op,
        ],
        { breakCoalescing: true },
      );
  ...
  const mergeBackward = useCallback(
    (block: Block) => {
      ...
      if (previous.type === "divider") {
        dispatchBatch([{ type: "delete_block", opId: opId(), blockId: previous.id }], { breakCoalescing: true });
        requestFocus({ blockId: block.id, offset: 0, forceTextSync: true });
        return;
      }
      ...
      dispatchBatch(
        [
          { type: "update_block", opId: opId(), blockId: previous.id, properties: { text: blockText(previous) + text } },
  ...
  const indentBlock = useCallback(
    (block: Block) => {
      ...
      dispatchBatch(
        [{ type: "move_block", opId: opId(), blockId: block.id, newParentId: previousSibling.id, index: previousSibling.content.length }],
        { breakCoalescing: true },
      );
  ...
  const selectSlashType = useCallback(
    (type: BlockType) => {
      ...
      const removal = removeSlashQuery(element?.textContent ?? blockText(block), caret);
      ...
      if (type === "divider") {
        ...
        dispatchBatch(
          [
            { type: "update_block", opId: opId(), blockId: block.id, properties: { text: nextText } },
      ...
      requestFocus({ blockId: block.id, offset: removal?.slashIndex ?? nextText.length, forceTextSync: true });
      dispatchBatch(
        [
          {
            type: "update_block",
            ...
            blockId: block.id,
            blockType: type,
            properties: { text: nextText, checked: type === "to_do" ? false : null },
          },
        ],
        { breakCoalescing: true },
      );
    },
    [dispatchBatch, insertSibling, onSelectedBlockChange, requestFocus, slash, tree],
  );
  ...
  const moveFocus = useCallback(
    (fromBlockId: string, direction: -1 | 1) => {
      ...
      requestFocus({ blockId: next.id, offset: direction === -1 ? blockText(next).length : 0 });
    },
  ...
  const handleKeyDown = useCallback(
    (block: Block, event: KeyboardEvent<HTMLElement>) => {
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "z") {
        ...
        setSlash(null);
        requestFocus({ blockId: block.id, offset: Math.min(getCaretOffset(event.currentTarget), blockText(block).length), forceTextSync: true });
      ...
      if (slash?.blockId === block.id) {
        ...
        if (event.key === "ArrowDown") {
          event.preventDefault();
          setSlash({ ...slash, activeIndex: itemCount === 0 ? 0 : (slash.activeIndex + 1) % itemCount });
          return;
        }
        if (event.key === "ArrowUp") {
          event.preventDefault();
          setSlash({ ...slash, activeIndex: itemCount === 0 ? 0 : (slash.activeIndex - 1 + itemCount) % itemCount });
          return;
        }
        if (event.key === "Enter" && itemCount > 0) {
          event.preventDefault();
          selectSlashType(filteredSlashItems(slash.query)[slash.activeIndex]?.type ?? "paragraph");
          return;
        }
        if (event.key === "Escape") {
          event.preventDefault();
      ...
      if (block.type === "code" && event.key === "Enter" && !event.shiftKey) {
        event.preventDefault();
        const text = blockText(block);
        requestFocus({ blockId: block.id, offset: caret + 1, forceTextSync: true });
        dispatchBatch([{ type: "update_block", opId: opId(), blockId: block.id, properties: { text: `${text.slice(0, caret)}\n${text.slice(caret)}` } }], {
          coalesceKey: `text:${block.id}`,
        });
        return;
      }
      if (block.type === "code" && (event.key === "Escape" || (event.key === "Enter" && event.shiftKey))) {
        event.preventDefault();
        exitCodeBlock(block);
        return;
      }
  ...
  const handleDragOver = useCallback(
    (event: DragEvent, block: Block) => {
      if (!draggingId || draggingId === block.id || isDescendantOf(tree, block.id, draggingId)) return;
      event.preventDefault();
      const rect = event.currentTarget.getBoundingClientRect();
      setDrop({ blockId: block.id, position: event.clientY < rect.top + rect.height / 2 ? "above" : "below" });
    },
    [draggingId, tree],
  );
  ...
  const handleDrop = useCallback(
    (event: DragEvent, target: Block) => {
      event.preventDefault();
      if (!draggingId || !drop || draggingId === target.id || !target.parentId) return;
      ...
      const rawIndex = targetParent.content.indexOf(target.id) + (drop.position === "below" ? 1 : 0);
      const oldIndex = siblingIndex(tree, dragged);
      const adjustedIndex = dragged.parentId === targetParent.id && oldIndex < rawIndex ? rawIndex - 1 : rawIndex;
      setDraggingId(null);
      setDrop(null);
      if (dragged.parentId === targetParent.id && oldIndex === adjustedIndex) return;
      dispatchBatch(
        [{ type: "move_block", opId: opId(), blockId: draggingId, newParentId: targetParent.id, index: adjustedIndex }],
        { breakCoalescing: true },
      );
      focusVisible(draggingId, isTextBlock(dragged) ? blockText(dragged).length : 0);
    },
  ...
  const renderBlock = (block: Block, depth: number): React.ReactNode => {
    ...
    const text = blockText(block);
    const showPlaceholder = block.type === "paragraph" && isFocused && text.length === 0;
    const blockSlash = slash?.blockId === block.id ? slash : null;
    const isCollapsed = collapsed.has(block.id);
    const children = getBlock(tree, block.id).content.map((childId) => getBlock(tree, childId));
    const checked = block.properties.checked === true;

    return (
      <div key={block.id} className="relative" style={{ paddingLeft: depth > 0 ? 24 : 0 }}>
        {drop?.blockId === block.id && drop.position === "above" ? <div className="h-0.5 rounded bg-sky-500" /> : null}
        <div
        ...
        >
          <button
            ...
            className="absolute left-1 top-1.5 hidden h-6 w-6 cursor-grab items-center justify-center rounded text-zinc-300 hover:bg-zinc-100 hover:text-zinc-600 group-hover:flex"
            onDragStart={(event) => {
          ...
          {block.type === "divider" ? (
            <div
              ...
              onKeyDown={(event) => {
                if (event.key === "Backspace") {
                  event.preventDefault();
                  dispatchBatch([{ type: "delete_block", opId: opId(), blockId: block.id }], { breakCoalescing: true });
                }
              }}
            >
            ...
            <div className="relative flex items-start gap-2">
              {block.type === "bulleted_list_item" ? <span className="mt-1.5 w-4 text-center text-zinc-500">•</span> : null}
              {block.type === "numbered_list_item" ? (
                <span className="mt-0.5 w-6 text-right text-zinc-500">{numberedValue(tree, block)}.</span>
              ) : null}
              {block.type === "to_do" ? (
                <input
                  type="checkbox"
                  ...
                  onChange={(event) =>
                    dispatchBatch(
                      [{ type: "update_block", opId: opId(), blockId: block.id, properties: { checked: event.currentTarget.checked } }],
                      { breakCoalescing: true },
                    )
                  }
                />
              ...
              {block.type === "callout" ? <span className="mt-1 text-base">💡</span> : null}
              <div className="relative min-w-0 flex-1">
                {showPlaceholder ? (
                  <span className="pointer-events-none absolute left-0 top-0 text-zinc-400">
                ...
                <div
                  ...
                  onFocus={(event) => {
                    ...
                    onSelectedBlockChange(block.id);
                    setElementText(event.currentTarget, text);
                  }}
                  onBlur={() => {
                    setFocusedBlockId((current) => (current === block.id ? null : current));
                    dispatchBatch([], { breakCoalescing: true });
                  }}
                  onInput={(event: FormEvent<HTMLElement>) => handleInput(block, event.currentTarget)}
                ...
                {blockSlash ? (
                  <SlashMenu
                    ...
                    onHover={(activeIndex) => setSlash({ ...blockSlash, activeIndex })}
        ...
        {drop?.blockId === block.id && drop.position === "below" ? <div className="h-0.5 rounded bg-sky-500" /> : null}
        {block.type === "toggle" && isCollapsed ? null : children.map((child) => renderBlock(child, depth + 1))}
      </div>
    );
  };

  return <div className="space-y-0.5">{getBlock(tree, tree.rootId).content.map((childId) => renderBlock(getBlock(tree, childId), 0))}</div>;
}
