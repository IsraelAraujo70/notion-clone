...
fn compile_operation(
    ...
    match operation {
        OperationDraft::InsertBlock {
            ...
            normalize_generated_properties(block.block_type, &mut properties);
...
fn normalize_generated_properties(block_type: BlockType, properties: &mut Map<String, Value>) {
    ...
    if !properties.contains_key(target) {
        if let Some(value) = properties.remove(fallback) {
