...
> I Built a Notion + Linear Alternative With AI

Alternative titles:

- I Cloned Notion and Linear Because Google Drive Wasn't Hard Enough
- Can AI Build Real Software? I Built Notion + Linear to Find Out
- After Google Drive, I Built a Notion + Linear Alternative
...
the right, with small Notion and Linear references pointing toward the product
and a subtle `EP. 2` badge.

Core promise: episode one used a Google Drive clone to test whether AI could help
one engineer build real software without replacing engineering fundamentals.
Episode two raises the difficulty: move from storing files to synchronizing
shared, structured state. Reason starts as a Notion clone and grows into a
Notion + Linear alternative with blocks, databases, Kanban, real-time
collaboration, permissions, AI, mobile, MCP, and code review.
...
### 0:00 to 0:45: Cold open

**ON CAMERA**

In the last video, I got fired and cloned Google Drive.

At the end of that video, I said it was the first project in a series where I
would clone hard software.

Unfortunately, I was serious.

Because for project number two, I tried to build Notion.

Then I added databases and Kanban, so now it is also a little bit Linear.

Not just the editor UI. I wanted blocks, nested pages, real-time collaboration,
permissions, search, AI, mobile, and eventually even pull request review.
...
The Google Drive clone was about files, bytes, and storage.

This one made every keystroke a distributed systems problem.

### 0:45 to 1:50: The sequel and the first lie

**SCREEN**

Show the final line from episode one:

> I'm going to clone hard software.

Then cut to the historical `challenge.md`:
...
The first project asked whether AI could help me build a real file product
without replacing architecture, authorization, storage, debugging, or tests.

Google Drive gave me resumable uploads, object storage, byte verification,
sharing, search, trash, sync, and worker jobs.

For the second project, I wanted to keep the same rule and change the kind of
difficulty. Files mostly sit still. A collaborative document changes every time
...
### 1:50 to 3:25: Everything is a block
...
### 3:25 to 4:45: Real-time is a recovery problem
...
### 4:45 to 5:45: Permissions have to follow the data
...
### 5:45 to 7:40: AI without a second write path
...
### 7:40 to 9:35: Scope creep montage
...
### 9:35 to 10:20: Building this with coding agents
...
Like the Google Drive clone, I built this with coding agents.

They were a force multiplier, especially for implementation and test coverage.
They were not a substitute for architecture or review. An agent can produce a
lot of code very quickly, including a lot of code that violates the same
invariant in five different places.
...
That is the idea behind this whole series: AI does not replace software
engineering fundamentals. It multiplies them.

If the architecture is unclear, AI creates unclear code faster. If permissions
are incomplete, AI creates more ways to leak data. If the tests are shallow, AI
helps you ship bugs with confidence.

Using AI well is not accepting code faster. It is making the constraints
explicit enough that bad changes are easy to detect and good changes have
evidence.

### 10:20 to 12:15: The bugs taught me more than the features
...
### 12:15 to 13:20: What I would change
...
### 13:20 to 14:20: What project two proved

**ON CAMERA**

The Google Drive clone gave me an answer to the first uncomfortable question:
yes, I can still build serious software.

Reason gave me a better question:

Can I keep doing it when the state is shared, the product is ambiguous, and
every new client has to preserve the same rules?

This time, the evidence is a sync protocol, the trade-offs inside it, the bugs
that violated it, the migrations that repaired persisted data, and the tests
that prove the important paths.

It did not make impostor syndrome disappear. It made the argument more specific.
And specific doubts can be tested.

Getting laid off still sucks. Building a product does not magically fix that.
Building two reminded me that engineering is not memorizing every answer. It is
...
### 14:20 to 15:10: Episode close and hiring pitch

**ON CAMERA**

This is project number two in the Clone Hard Software series.

The first project was a Google Drive clone: files, storage, uploads, and
authorization.

The second is Reason: blocks, collaborative state, databases, AI, and a product
that got much bigger than the challenge.

The point is not that one developer can recreate years of work from the teams
behind Google Drive, Notion, or Linear.

The point is that AI gives engineers more leverage, and leverage only helps when
you understand what correct means.
...
And whether you are hiring or not, tell me what hard software I should clone for
project number three.

I can also make a deeper video about the sync engine, the AI operation flow, or
all the ways the editor broke before it started looking obvious.
...
This is episode two of Clone Hard Software.

In episode one, I got laid off and built a Google Drive clone to test whether AI
could help one engineer build real, product-shaped software without replacing
engineering fundamentals.

For project two, I built Reason. It started as a Notion clone and grew into a
Notion + Linear alternative: a block-based collaborative workspace with a Rust
backend, Next.js and React clients, PostgreSQL and pgvector, real-time operation
sync, AI writes with approval, Android, an Electron experiment, MCP access,
inline databases, Kanban, and GitHub pull request review.
...
The series has one thesis: AI does not replace software engineering
fundamentals. It multiplies them. If you understand architecture, testing,
authorization, debugging, and product thinking, AI makes you faster. If you do
not, AI helps you ship broken software faster.
...
```text
00:00 After the Google Drive clone
00:45 The sequel and the first lie
01:50 Everything is a block
03:25 Real-time is a recovery problem
04:45 Permissions follow the data
05:45 AI without a database shortcut
07:40 The scope creep montage
09:35 Building this with coding agents
10:20 The bugs taught me more
12:15 What I would change
13:20 What project two proved
14:20 What should I clone next?
```
...
- Open with a two- or three-second callback to the Google Drive video before
  showing Reason. The callback establishes the series without turning this into
  a recap.
- Treat the Drive-to-Reason transition as the technical escalation: stored
  bytes became shared mutable state.
